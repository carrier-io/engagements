...
from plugins.engagements.utils.utils import make_unique_filename
import flask_restful  # pylint: disable=E0401

from pylon.core.tools import log

from tools import auth, api_tools, MinioClient  # pylint: disable=E0401
from plugins.engagements.serializers.attachment import attachments_schema
from plugins.engagements.utils.utils import generate_thumbnail


class API(flask_restful.Resource):  # pylint: disable=R0903
    """ API Resource """

    url_params = ['<int:project_id>/<int:engagement_id>']
    ...
    @auth.decorators.check_api({
        "permissions": ["engagements.engagements.attachments.view"],
        "recommended_roles": {
            "administration": {"admin": True, "viewer": False, "editor": True},
            "default": {"admin": True, "viewer": False, "editor": True},
            "developer": {"admin": True, "viewer": False, "editor": True},
        }})
    def get(self, project_id, engagement_id):
        """ Get all attachments"""
        attachments = self.module.list_ticket_attachments(project_id, engagement_id)
        attachments = attachments_schema.dump(attachments)
        return {"ok": True, "items":attachments}


    @auth.decorators.check_api({
        "permissions": ["engagements.engagements.attachments.create"],
        "recommended_roles": {
            "administration": {"admin": True, "viewer": False, "editor": True},
            "default": {"admin": True, "viewer": False, "editor": True},
            "developer": {"admin": True, "viewer": False, "editor": True},
        }})
    def post(self, project_id, engagement_id):
        """ Create attachments"""
        ...
        attachments = []
        for file in files:
            bucket = "engagements-attachments"
