...
class Slot:  # pylint: disable=E1101,R0903
    ...
    def _content(self, context, slot, payload):
        _ = slot
        #
        id = payload.request.args.get('id')
        if not id:
            return theme.empty_content
        engagement = self.get_engagement_or_404(id)
        #
        with context.app.app_context():
            return self.descriptor.render_template(
                "engagement/content.html", engagement=engagement,
    ...
    def _scripts(self, context, slot, payload):
        ...
        with context.app.app_context():
            return self.descriptor.render_template(
                "engagement/scripts.html", engagement=engagement,
                project_id=session_project.get()
            )
