...
from plugins.engagements.models.attachments import Attachment
from ..utils.logs import log_attachment_create, log_attachment_delete, log_attachment_update


class RPC:  # pylint: disable=E1101,R0903
    """ RPC Resource """

    @web.rpc("engagements_list_ticket_attachments", "list_ticket_attachments")
    @rpc_tools.wrap_exceptions(RuntimeError)
    def _list_ticket_attachments(self, project_id, hash_id):
        return Attachment.query.filter_by(project_id=project_id, engagement_id=hash_id)

    @web.rpc("engagements_add_batch_attachments", "add_batch_attachments")
    @rpc_tools.wrap_exceptions(RuntimeError)
    ...
    @web.rpc("engagements_update_attachment", "update_attachment")
    @rpc_tools.wrap_exceptions(RuntimeError)
    def _update_attachment(self, project_id, id, payload):
        ...
        for field, value in payload.items():
            obj_value = getattr(attachment, field)
    ...
    @web.rpc("engagements_delete_attachment", "delete_attachment")
    @rpc_tools.wrap_exceptions(RuntimeError)
    def _delete_attachment(self, project_id, id):
        ...
        self.context.event_manager.fire_event(
            'engagements_attachment_deleted', 
